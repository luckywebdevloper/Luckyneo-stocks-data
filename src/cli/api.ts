...
export async function showIndexOverview() {
  ...
  spinner.start();
  const { data: allIndexData } = await nse.getDataByEndpoint(
    ApiList.ALL_INDICES
  );
  spinner.text = "";
  spinner.stop();
...
export async function showIndexDetails(argv: any) {
  ...
  if (data) {
    ...
    console.log(
      chalk.red(
        `${index} index symbol is invalid. Try to enclose the index symbols with quotes.`
      )
    );
  }
}
...
export async function showEquityDetails(argv: any) {
  ...
  spinner.start();
  try {
    const { info, priceInfo, metadata, securityInfo } =
    ...
    if (info && marketDeptOrderBook) {
      ...
      const changeStatus =
        changePrice <= 0
          ? `${rupee}${changePrice} (${changePercent}%)${chalk.red("▼")}`
          : `${rupee}${changePrice} (${changePercent}%)${chalk.green("▲")}`;

      const derivativeStatus =
        securityInfo.derivatives === "Yes"
          ? chalk.black.bgGreen(" Derivatives ")
          : chalk.black.bgRed(" Derivatives ");

      const slbStatus =
        securityInfo.slb === "Yes"
          ? chalk.black.bgGreen(" SLB ")
          : chalk.black.bgRed(" SLB ");

      const ltpStatus =
        changePrice <= 0
          ? chalk.black.bgRed(` LTP: ${priceInfo.lastPrice} `)
...
export async function showHistorical(argv: any) {
  ...
  console.log();
  console.log(
    asciichart.plot(CloseData, { ...chartConfig, colors: [asciichart.cyan] })
  );
  console.log();
  console.log(
    chalk.black.bgYellow(" Volume Chart - In Lakhs (Last 3 months) ")
  );
  console.log();
  console.log(
    asciichart.plot(Volume, { ...chartConfig, colors: [asciichart.yellow] })
  );
  console.log();
